...
PKGS="wget gnome-icon-theme software-properties-common \
        ...
        gir1.2-gtk-3.0 libxv1 libglu1-mesa bzip2"
if [ "$VERSION_ID" == "20.04" ]; then
    PKGS+=" firefox"
fi
if [ "$VERSION_ID" == "22.04" ] || [ "$VERSION_ID" == "24.04" ]; then
    PKGS+=" libxtst6 dbus-x11"
fi
...
if [ "$VERSION_ID" == "22.04" ] || [ "$VERSION_ID" == "24.04" ]; then
    wget  --no-verbose --show-progress --progress=dot:giga \
        "https://download.mozilla.org/?product=firefox-latest-ssl&os=linux64&lang=en-US" -O firefox.tar
